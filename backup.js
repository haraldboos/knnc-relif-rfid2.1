            ...
            // }
                ...
                dani.query(cheacksql ,rfidtag,(error,reasult)=>{
                    ...
                    if (reasult.length > 0 ){ 
                        ...
                        upldate = `update ${fdate} set atamp = 1, atmtime = '${time}'  where (leasson <= ${timex} and atamp = 0 and subteacher = '${tename}') limit 1 ; `
...
web.post('/uploding',(req,res,next)=>{
    ...
    //uploadfile
    const uploadtimetable = multer({storage:stor}).single('timexlfile');
    ...
    const createTableQuery = `CREATE TABLE IF NOT EXISTS ${fdate} (tname VARCHAR(150) NOT NULL,reason VARCHAR(45) NOT NULL,leasson INT NOT NULL,class VARCHAR(170) NOT NULL,subject VARCHAR(160) NOT NULL,concad VARCHAR(45) NOT NULL,subteacher VARCHAR(150) NOT NULL,atamp INT NOT NULL DEFAULT 0,relifno INT NOT NULL AUTO_INCREMENT,PRIMARY KEY (relifno), atmtime VARCHAR(6))`;
    const timeandtablen = `$fdate`;
    const cretetimetable = `CREATE TABLE ${timeandtablen} (
        pediod_no INT NOT NULL,
        time INT NOT NULL,
        PRIMARY KEY (pediod_no),
...
`;
    dani.query(tablecheack,[mydbname,fdate],(err,reasult)=>{
        ...
        const getdata = `select relifno,tname,subteacher,leasson,class,subject,atamp,atmtime from ${mydbname}.${fdate};`; 
